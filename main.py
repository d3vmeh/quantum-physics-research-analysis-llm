...
#Use keyword, vector search, and grapoh search to retrieve information
#Combine the information sources, pass as context to the LLM to get an answer
#Return the answer to the user

#Data-->embeddings-->vector database stored in NEo4j
#keybaord search and similarity search
#then use knowledge graph to fetch information
#all of the above combined to create context
#pass context to LLM to get answer
...
df = pd.read_csv("quantum_physics_articles.csv")

print(df.head())
print("=====================================")
print(type(list(df["abstract"])))
print("Documents Loaded")


raw_data = list(df["abstract"]) #List of documents to be processed

#Need to split the document into chunks
#print("Splitting documents into chunks")
#text_splitter = TokenTextSplitter(chunk_size = 512, chunk_overlap = 24)
#documents = text_splitter.split_documents(raw_data) #only collecting the first three documents
#print("Documents Split")

documents = []
print("Converting data to documents")

print("Number of abstracts:",len(raw_data))
count = 0
#f = open("q1.txt",'w')
for d in raw_data:
    #f.write(d+'\n\n')
...
graph_documents = llm_transformer.convert_to_graph_documents(documents[:200])
